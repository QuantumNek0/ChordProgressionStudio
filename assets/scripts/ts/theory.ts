...
const sharpToFlat: Map<string, string> = new Map([
...
// Map for note names to MIDI values
const noteToMidi: Map<string, number> = new Map([
...
const midiToNote: Map<number, string> = new Map([
...
const octaveRange: Map<number, [start: number, end: number]> = new Map([
    [2, [36, 47]],
    [3, [48, 59]],
    [4, [60, 71]],
    [5, [72, 83]],
...
const ScaleChords: Map<string, string[]> = new Map([
...
export function getSharpToFlat(note: string): string {
    return sharpToFlat.get(note);
}

// Function to get MIDI value from note name
export function getMidiValue(note: string): number {
...
export function getOctaveRange(octave: number): [start: number, end: number] {
    return octaveRange.get(octave);
}

// Function to get scale chords
export function getScaleChords(key: string): string[] {
